const DEFAULT_API_BASE = 'https://server-pied-nu.vercel.app/api';
...
export const adminApi = {
  ...
  students: { list: (params?: Record<string, string>) => api.get<ApiResponse<{ students: Student[]; total: number }>>(`/students?${new URLSearchParams(params || {})}`), getProfile: (id: string) => api.get<ApiResponse<StudentAcademicProfile>>(`/students/${id}/profile`), create: (data: Record<string, unknown>) => api.post<ApiResponse<unknown>>('/students', data), update: (id: string, data: Record<string, unknown>) => api.put<ApiResponse<unknown>>(`/students/${id}`, data), delete: (id: string) => api.delete<ApiResponse<unknown>>(`/students/${id}`), import: (rows: Record<string, unknown>[]) => api.post<ApiResponse<{ imported: number; skipped: number }>>('/students/import', { rows }) },
...
export const riskApi = {
  getDashboard: () => api.get<ApiResponse<AdvisorRiskDashboard>>('/risk/dashboard'),
  ...
  getStudentRiskDetails: (studentId: string) => api.get<ApiResponse<StudentRiskDetails>>(`/risk/student/${studentId}`),
  getMyRiskStatus: () => api.get<ApiResponse<RiskStatus>>('/risk/my-status'),
  getRiskStats: () => api.get<ApiResponse<RiskStats>>('/risk/stats'),
};

export const aiApi = {
  getStudentSummary: (studentId: string) => api.get<ApiResponse<AiStudentAdvisingSummary>>(`/ai/students/${studentId}/summary`),
};
...
export const advisingApi = {
  getMyAdvisorProfile: () => api.get<ApiResponse<Advisor>>('/advising/advisors/me'),
  getDashboard: () => api.get<ApiResponse<AdvisorDashboardSummary>>('/advising/dashboard'),
  ...
  updateSession: (id: string, data: { scheduledAt?: string; durationMinutes?: number; sessionType?: string; location?: string; meetingLink?: string; status?: SessionStatus; summary?: string; recommendations?: string; followUpRequired?: boolean; followUpDate?: string | null }) => api.patch<ApiResponse<AdvisingSession>>(`/advising/sessions/${id}`, data),
  updateSessionStatus: (id: string, data: { status: SessionStatus; summary?: string; recommendations?: string; followUpRequired?: boolean; followUpDate?: string | null }) => api.patch<ApiResponse<AdvisingSession>>(`/advising/sessions/${id}/status`, data),
  getSessionNotes: (sessionId: string) => api.get<ApiResponse<{ notes: SessionNote[] }>>(`/advising/sessions/${sessionId}/notes`),
  addSessionNote: (sessionId: string, data: { noteType?: SessionNoteType; content: string; isPrivate?: boolean }) => api.post<ApiResponse<SessionNote>>(`/advising/sessions/${sessionId}/notes`, data),
  addNote: (data: { sessionId: string; noteType?: SessionNoteType; content: string; isPrivate?: boolean }) => api.post<ApiResponse<SessionNote>>('/advising/sessions/notes', data),
};
...
export type DashboardRiskLevel = 'GOOD' | 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';
...
export interface AdvisorDashboardUpcomingSession {
  id: string;
  studentId: string;
  studentName: string;
  studentCode: string;
...
export interface AdvisorDashboardAlert {
  id: string;
  type: 'RISK_ALERT';
  studentId: string;
  studentName: string;
...
export interface AdvisorDashboardSummary {
  ...
  upcomingSessions: AdvisorDashboardUpcomingSession[];
  recentAlerts: AdvisorDashboardAlert[];
  unreadNotifications: number;
}

export interface RiskDashboardStudent {
  studentId: string;
  studentName: string;
  studentCode: string;
...
export interface RiskDashboardAlert {
  id: string;
  studentId: string;
  studentName: string;
  studentCode: string;
...
export interface AdvisorRiskDashboard {
  totalStudents: number;
  studentsByRiskLevel: RiskLevelDistribution;
  highRiskStudents: RiskDashboardStudent[];
  lowAttendanceStudents: RiskDashboardStudent[];
  recentAlerts: RiskDashboardAlert[];
}
...
export type SessionTimeframe = 'UPCOMING' | 'PAST';
...
export interface StudentAcademicProfileCourse {
  courseId: string;
  courseCode: string;
  courseName: string;
  credits: number | null;
...
export interface StudentAcademicProfileAdvisor {
  advisorId: string;
  doctorId: string;
  doctorName: string;
  specialization: string;
...
export interface StudentAcademicProfileSession {
  id: string;
  scheduledAt: string;
  status: string;
  sessionType: string;
...
export interface StudentAcademicProfile {
  ...
  currentCourses: StudentAcademicProfileCourse[];
  failedCourses: Array<{
    courseId: string;
    courseCode: string;
    courseName: string;
  ...
  advisor: StudentAcademicProfileAdvisor | null;
  risk: {
    level: string;
    averageAttendance: number;
  ...
  advisingSessions: StudentAcademicProfileSession[];
  unreadNotifications: number;
  graduationProgress: {
...
export interface AiStudentAdvisingSummaryUpcomingSession {
  id: string;
  scheduledAt: string;
  status: string;
  sessionType: string;
...
export interface AiStudentAdvisingSummary {
  riskLevel: string;
  attendanceRate: number;
  attendanceStatus: 'GOOD' | 'WARNING' | 'LOW';
  missedSessionsCount: number;
  upcomingAdvisingSessions: AiStudentAdvisingSummaryUpcomingSession[];
  unreadNotifications: number;
