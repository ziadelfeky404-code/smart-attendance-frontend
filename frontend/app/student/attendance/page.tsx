...
export default function StudentAttendancePage() {
  ...
  return (
    <div className="min-h-screen bg-dark" data-testid="student-attendance-page">
      <Navbar />
      <main className="lg:pr-64 pt-24 p-6 max-w-2xl mx-auto">
        ...
        {step === 'sections' && (
          <div className="space-y-3" data-testid="student-sections-list">
            {sections.length === 0 ? (
              ...
              sections.map(s => (
                <button key={s.section_id} onClick={() => selectSection(s)} className="card w-full text-right hover:border-primary/30 transition-all" data-testid="student-section-option">
        ...
        {step === 'scan' && session && (
          <div className="space-y-6">
            <div className="card">
              ...
              <button onClick={handleSendOtp} disabled={otpLoading} className="btn-primary w-full flex items-center justify-center gap-2" data-testid="student-send-otp-button">
                {otpLoading ? '...' : <><Send size={18} /> إرسال رمز التحقق</>}
              </button>
              {otpError && !otpSent && (
                <p className="text-danger text-sm mt-3 flex items-center gap-1" data-testid="student-otp-error">
                  <AlertCircle size={14} />
                  {otpError}
                </p>
              )}
            ...
            {otpSent && (
              <div className="card">
                ...
                <input
                  ...
                  dir="ltr"
                />
                {otpError && <p className="text-danger text-sm mt-2 flex items-center gap-1" data-testid="student-otp-error"><AlertCircle size={14} />{otpError}</p>}
            ...
            <button onClick={() => setStep('sections')} className="btn-secondary w-full" data-testid="student-back-to-sections-button">العودة للشعب</button>
          </div>
        )}

        {step === 'success' && result && (
          <div className="card text-center" data-testid="student-attendance-success">
        ...
        {step === 'error' && (
          <div className="card text-center" data-testid="student-attendance-error">
