...
export default function DoctorDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      const [sectionsRes, sessionsRes] = await Promise.all([
        adminApi.sections.list({ doctorId: user!.id, limit: '50' }),
  ...
  return (
    <div className="min-h-screen bg-dark" data-testid="doctor-dashboard">
      <Navbar />
          ...
          <>
            {sessions.length > 0 && (
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" data-testid="doctor-active-sessions">
                  {sessions.map(s => (
            ...
            <div className="card" data-testid="doctor-open-session-form">
              ...
              <div className="mb-4">
                ...
                <select value={openSection} onChange={e => setOpenSection(e.target.value)} className="input-field" data-testid="doctor-section-select">
              ...
              <button onClick={openSession} disabled={!openSection || (gpsEnabled && (!gpsData.lat || !gpsData.lon))} className="btn-primary w-full md:w-auto text-lg py-3" data-testid="doctor-open-session-button">
      ...
      {qrSession && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && setQrSession(null)}>
          <div className="card text-center" data-testid="doctor-qr-modal">
      ...
      {showAttendance && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && setShowAttendance(null)}>
          <div className="card w-full max-w-3xl max-h-[80vh] overflow-y-auto" data-testid="doctor-attendance-modal">
