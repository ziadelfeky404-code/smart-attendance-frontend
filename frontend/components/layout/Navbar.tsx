...
export default function Navbar() {
  ...
  const getNavTestId = (href: string) => {
    const normalized = href.replace(/^\//, '').replace(/\//g, '-');
    return `nav-link-${normalized || 'home'}`;
  };

  return (
    <>
      <nav className="fixed top-0 right-0 left-0 z-50 glass border-b border-dark-300" data-testid="app-navbar">
        <div className="flex items-center justify-between px-4 lg:px-6 h-16">
          ...
          <div className="flex items-center gap-3">
            ...
            <div ref={profileRef} className="relative">
              <button onClick={() => setProfileOpen(!profileOpen)} className="flex items-center gap-2 p-1.5 rounded-xl hover:bg-dark-200 transition-colors" data-testid="profile-menu-button">
              ...
              {profileOpen && (
                <div className="absolute left-0 top-full mt-2 w-56 card p-2 shadow-xl z-50">
                  ...
                  <button onClick={logout} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-danger hover:bg-danger/10 transition-colors text-sm" data-testid="logout-button">
      ...
      <aside className={`fixed top-16 right-0 bottom-0 w-64 bg-dark-100 border-l border-dark-300 z-40 transform transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <nav className="p-4 space-y-1 overflow-y-auto h-full">
          {items.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`nav-link ${isActive ? 'active' : ''}`} data-testid={getNavTestId(item.href)}>
                <Icon size={20} />
